...
class LP:
    def __init__(self, low_delta_list, delta_truncation, delta_spacing, delta_max, 
        ...
        self.derivative_constraints = DG("LP_data/isingDGn_v5.csv", self.delta_list, 3, how_many_constraints)*np.exp(-rescale_factor*np.array(self.delta_list))
        self.solver = np.array([2]+[0 for _ in range(how_many_constraints-1)])
        
        self.a1_eq = np.array(list(DG("LP_data/isingDGn_v5.csv", self.low_delta_list, 3, 1)[0]*np.exp(-rescale_factor*np.array(self.low_delta_list))) + 
